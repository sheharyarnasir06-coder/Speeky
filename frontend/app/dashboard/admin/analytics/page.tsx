...
/**
 * Platform Analytics — PAD-US-10..14's live Overview/Funnel/Feature-Usage/
 * Cross-Filter/Revenue tabs, US-206 saved views, US-2xx reconciliation, plus
 * a "Growth Signals" tab (GAP-03/GAP-04's signup/retention/churn/revenue time
 * series) that every anomaly alert deep-links into pre-filtered
 * (?metric=&from=&to=) — per GAP-03's acceptance criterion that an alert
 * never opens the generic homepage.
 */
export default function AdminAnalyticsPage() {
  ...
  const [tab, setTab] = React.useState<Tab>(focusedMetric ? "signals" : "overview");
        ...
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            {focusedMetric
              ...
              : "Platform activity, onboarding drop-off, feature adoption, payment reconciliation, and the growth signals that feed anomaly alerts and scheduled reports."}
            {tab !== "signals" && isSuperAdmin ? " Revenue is a Super Admin-only demo view — no billing is connected yet." : ""}
          </p>
...
// ── Growth Signals (GAP-03 / GAP-04) — the shared platform_metrics_service
// pipeline anomaly alerts and scheduled reports both read from ───────────────
function SignalsTab({ focusedMetric, dateFrom, dateTo }: { focusedMetric: MetricKey | null; dateFrom?: string; dateTo?: string }) {
  ...
  // with its anomalous window shaded — the rest follow as small multiples for
  // context. Each metric is its own single-series chart (never a shared axis
  // — the metrics have incompatible scales: counts, percents, currency).
