...
type Step =
  ...
  | { name: "attempt-result"; session: PronunciationSession; result: AttemptResult }
...
export default function PronunciationCoachPage() {
    ...
    checkResumableSession()
      ...
      .catch((err) => {
        if (!cancelled) {
          setStep({
            name: "error",
            message: err instanceof ApiError ? err.message : "Couldn't load Pronunciation Coach.",
          });
        }
      });
    return () => {
  ...
  React.useEffect(() => {
    function handleHidden() {
      ...
      if (document.visibilityState === "hidden" && active && !active.completed) {
  ...
  async function handleResume(check: ResumeCheck) {
    ...
    try {
      ...
      activeSessionRef.current = { sessionId: resumed.session_id, completed: false };
      setStep({
        name: "practice",
        session: {
  ...
  if (!isUnlocked) {
    return (
      <div className="flex items-start gap-2.5 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-foreground">
        ...
        {access?.locked_message ?? "Complete your baseline assessment to unlock Pronunciation Coach."}
      </div>
    );
  }
      ...
      <div className="mx-auto flex max-w-lg flex-col gap-4">
        ...
        <div className="flex flex-col items-start gap-3 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 gap-2">
            <Button size="sm" variant="outline" loading={isSubmitting} onClick={handleDiscard}>
              Start Fresh
            </Button>
            <Button size="sm" loading={isSubmitting} onClick={() => handleResume(step.check)}>
              Resume
            </Button>
          </div>
        </div>
  ...
  if (step.name === "practice") {
    ...
    return (
      <div className="mx-auto flex max-w-2xl flex-col gap-6">
        {gate}
        <div>
          <h1 className="font-serif text-h2 font-semibold text-foreground">Pronunciation Coach</h1>
        ...
        <div className="rounded-2xl border border-border bg-surface-elevated p-6 shadow-sm">
          ...
          <p className="mt-4 font-serif text-xl font-semibold text-foreground">{session.sentence}</p>

          {recorder.error ? <p className="mt-3 text-sm text-danger">{recorder.error}</p> : null}
          ...
          <div className="mt-5 flex items-center gap-3">
            <button
              ...
              className={cn(
                ...
                recorder.isRecording ? "animate-pulse bg-danger" : "bg-primary hover:bg-primary-hover",
              )}
              aria-label={recorder.isRecording ? "Stop recording and submit" : "Record your attempt"}
            >
              {recorder.isRecording ? (
      ...
      <div className="mx-auto flex max-w-2xl flex-col gap-6">
        ...
        >
          ...
          <div>
            <p>{result.message}</p>
            {result.transcript ? (
              <p className="mt-1 text-xs text-muted-foreground">We heard: &quot;{result.transcript}&quot;</p>
            ) : null}
          </div>
        </div>
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <div className="animate-fade-up rounded-2xl border border-border bg-gradient-to-br from-primary to-primary-hover p-8 text-center text-primary-foreground shadow-sm">
        ...
        <h1 className="mt-3 font-serif text-h2 font-semibold">Session Complete</h1>
        <p className="mt-2 text-sm text-primary-foreground/85">
          {summary.attempt_count} sentence{summary.attempt_count === 1 ? "" : "s"} practiced.
        </p>
      </div>

      {summary.phoneme_accuracy.length > 0 ? (
        <div className="rounded-2xl border border-border bg-surface-elevated p-6 shadow-sm">
          <h2 className="font-serif text-lg font-semibold text-foreground">Accuracy by sound</h2>
          <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
            {summary.phoneme_accuracy.map((p) => (
              <div key={p.phoneme} className="rounded-xl border border-border bg-surface p-4">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  &quot;{p.phoneme}&quot; sound
                </p>
                <p className="mt-1 text-xl font-semibold text-foreground">
                  {p.total_words === 0 ? "—" : `${Math.round((p.correct_words / p.total_words) * 100)}%`}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">{p.attempts} attempt{p.attempts === 1 ? "" : "s"}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <Button size="lg" variant="outline" className="self-center" href="/dashboard/explore">
        Back to Explore
      </Button>
    </div>
  );
